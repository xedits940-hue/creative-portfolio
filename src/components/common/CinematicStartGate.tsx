...
export default function CinematicStartGate() {
  ...
  const toggleAudio = () => {
    ...
    if (nextState && audio.paused && activated) {
      audio.play().catch(() => {});
    }
  };
    ...
    <>
      <style jsx global>{`
        ...
        .vs-entry.is-opening .vs-entry__flash {
          animation: vsEntryFlash 0.78s cubic-bezier(0.76, 0, 0.24, 1)
            both;
        }
        ...
        .vs-gate:hover .vs-gate__surface::before {
          animation: vsSurfaceSweep 1.1s cubic-bezier(0.76, 0, 0.24, 1)
            both;
        }
        ...
        .vs-entry.is-opening .vs-gate {
          pointer-events: none;
          animation: vsGateOpen 0.78s cubic-bezier(0.76, 0, 0.24, 1)
            both;
        }
      ...
      >
        ...
        <button
          ...
          aria-label={audioOn ? "Mute intro sound" : "Enable intro sound"}
          onClick={toggleAudio}
        >
          <svg
            viewBox="0 0 24 24"
        ...
        <button
          ...
          onPointerMove={(event) => {
            ...
            gate.style.setProperty("--mx", `${x}%`);
            gate.style.setProperty("--my", `${y}%`);
          }}
          onPointerLeave={() => {
            const gate = gateRef.current;
            if (!gate) return;

            gate.style.setProperty("--mx", "50%");
            gate.style.setProperty("--my", "50%");
          }}
        >
